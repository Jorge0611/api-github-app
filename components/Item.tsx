...
const Item = ({ id, repos, ...props }: Props) => {
  const { name, full_name, owner, language, description, html_url } =
    ...
    <>
      ...
      >
        <div className="fixed top-0 left-0 right-0 m-6 mt-24 md:mt-12 md:m-24">
          <motion.div
            className="fixed top-0 right-0 mr-6 mt-12 md:mt-6"
            animate
          >
            <button
              {...props}
          ...
          >
            <div className="flex flex-row items-center justify-between">
              ...
              >
                <h3 className="text-xl md:text-4xl uppercase">{name}</h3>
                <p className="text-md">{full_name}</p>
              </motion.div>
            ...
            <motion.div layoutId={`card-description-${id}`} className="text-md	">
              <p>{description}</p>
            </motion.div>
